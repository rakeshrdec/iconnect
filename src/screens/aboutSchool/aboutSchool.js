...
const AboutSchool = ({ navigation }) => {
    ...
    const getAcademy = () => {
        fetch(apiUrl + `/acedemy/getAcedemyDetails`).then((res) => {
            res.json().then((data) => {
                if (data != '') {
                            ...
                            <Pressable style={{ elevation: 15, flexDirection: 'row', width: '90%', alignSelf: 'center', margin: 10, alignItems: 'center', backgroundColor: 'white', borderRadius: 15, padding: 10 }}>
                                <View style={{ marginHorizontal: 0, alignSelf: 'center', alignItems: 'center' }}>
                                    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                                        {academyDetails.logoImg === null ? <Image source={require('../../../assets/logo/male_student_avatar.png')} style={{ width: 70, height: 70, resizeMode: 'stretch', borderRadius: 150 }} /> : <Image source={{ uri: apiUrl + '/' + academyDetails.logoImg }} style={{ width: 70, height: 70, resizeMode: 'stretch', borderRadius: 150 }} />}
                                    </View>
